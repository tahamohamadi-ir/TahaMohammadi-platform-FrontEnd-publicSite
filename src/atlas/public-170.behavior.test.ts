...
describe.sequential('PUBLIC-170 behavior', () => {
  it('registers Atlas integration artifacts outside src/pages', () => {
    expect(
      existsSync(
        path.join(repositoryRoot, 'src', 'integrations', 'design-atlas.mjs'),
      ),
    ).toBe(true)
    expect(
      existsSync(path.join(repositoryRoot, 'src', 'atlas', 'AtlasRoute.astro')),
    ).toBe(true)
    expect(
      existsSync(
        path.join(repositoryRoot, 'docs', 'design', 'DESIGN-ATLAS.md'),
      ),
    ).toBe(true)
  ...
  it('exposes stable atlas and specimen visual ids', () => {
    ...
    const componentGallery = readRepositoryFile(
      'src/atlas/sections/ComponentGallery.astro',
    )
    const localeThemeSection = readRepositoryFile(
      'src/atlas/sections/LocaleThemeSection.astro',
    )
    const stateSheetSection = readRepositoryFile(
      'src/atlas/sections/StateSheetSection.astro',
    )

    expect(atlasPage).toContain('data-visual-id="VisualAtlas"')
    expect(componentGallery).toContain(
      'data-visual-id="AtlasComponentsSection"',
    )
    expect(componentGallery).toMatch(/AtlasSpecimen-/)
    expect(componentGallery).toContain('lucide-astro')
    expect(localeThemeSection).toContain(
      'data-visual-id="AtlasThemeSpecimen-light"',
    )
    expect(localeThemeSection).toContain(
      'data-visual-id="AtlasThemeSpecimen-dark"',
    )
    expect(localeThemeSection).toContain(
      'data-visual-id="AtlasThemeSpecimen-probe"',
    )
    expect(stateSheetSection).toContain(
      'data-visual-id="AtlasReducedMotionSpecimen"',
    )
    expect(stateSheetSection).toContain(
      'data-visual-id="AtlasReducedMotionProbe"',
    )
    expect(readRepositoryFile('src/components/ThemeToggle.astro')).toContain(
      'data-visual-id="ThemeToggle"',
    )
    expect(readRepositoryFile('src/components/Footer.astro')).toContain(
      'data-visual-id="Footer"',
    )
  })
  ...
  it('builds Atlas output only when DESIGN_ATLAS=1', () => {
    ...
    const outputFiles = readdirSync(path.join(repositoryRoot, 'dist'), {
      recursive: true,
    })
