...
describe('PUBLIC-240 pagefind build contract', () => {
  it('wires the pagefind integration and dependency', () => {
    ...
    expect(existsSync(path.join(repositoryRoot, 'src', 'integrations', 'pagefind.mjs'))).toBe(true);
    expect(existsSync(path.join(repositoryRoot, 'src', 'pages', 'en', 'search', 'index.astro'))).toBe(true);
    expect(existsSync(path.join(repositoryRoot, 'src', 'pages', 'fa', 'search', 'index.astro'))).toBe(true);
  });
});
