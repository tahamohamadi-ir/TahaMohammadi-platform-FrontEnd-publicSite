...
describe('PUBLIC-160 behavior', () => {
  it('registers all six template names from the pinned authority', () => {
    ...
    for (const name of templateNames) {
      expect(existsSync(path.join(repositoryRoot, 'src', 'components', 'templates', `${name}.astro`))).toBe(true);
    }
  });
  ...
  it('keeps exactly one H1 responsibility in a designated lead slot per template', () => {
    for (const name of templateNames) {
      ...
      expect(source, `${name} must delegate H1 ownership to a lead region`).toMatch(/h1-region|identity-lead/);
    }
  });
  ...
  it('renders detail templates with lead, body or media, and related slots', async () => {
    ...
    expect(evidenceHtml).toMatch(/data-visual-id="EvidenceVisualDetailTemplate"/);
