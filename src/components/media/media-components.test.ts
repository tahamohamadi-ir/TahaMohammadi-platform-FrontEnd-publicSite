...
import {
  ...
  PREVIEW_WIDTHS,
} from '../../lib/media/transform-recipes'
...
function parsePictureMarkup(html: string) {
  ...
  const sources = [...pictureHtml.matchAll(/<source\b[^>]*>/g)].map(
    (match) => match[0],
  )
...
function expectResponsivePicture(
  ...
  for (const format of formats) {
    const source = sources.find((markup) =>
      markup.includes(`type="image/${format}"`),
    )
...
describe('WP-30A media components', () => {
  ...
  it('keeps only one active theme mount target on initial load', async () => {
    ...
    const mountMatch = html.match(
      /<div data-theme-picture-mount[^>]*>[\s\S]*?<\/div>/,
    )
    expect(mountMatch?.[0] ?? '').not.toMatch(/<picture/)

    const templatePictures = [
      ...html.matchAll(
        /data-theme-picture-template="(light|dark)"[\s\S]*?<\/template>/g,
      ),
    ]
    ...
    const noscriptBlock =
      html.match(/<noscript>[\s\S]*?<\/noscript>/)?.[0] ?? ''
    expect(noscriptBlock.match(/<picture/g)?.length).toBe(1)

    const lightTemplates = html.match(
      /data-theme-picture-template="light"[\s\S]*?<\/template>/,
    )
    const darkTemplates = html.match(
      /data-theme-picture-template="dark"[\s\S]*?<\/template>/,
    )
  ...
  it('rejects reversed or mismatched ThemePicture light and dark asset ids', async () => {
    await expect(
      ...
      /lightAssetId home-graph-backplate-dark is registered for theme dark, not light/,
    )
    ...
    await expect(
      ...
      /lightAssetId portal-orbit-dark is registered for theme dark, not light/,
    )
  })
  ...
  it('pins atmosphere and preview width sets in rendered srcsets', async () => {
    ...
    const { img: previewImg } = expectResponsivePicture(
      previewHtml,
      PREVIEW_WIDTHS,
    )
