...
describe('PUBLIC-140 behavior', () => {
  it('renders LocalTabs as URL-backed nav links with active aria-current only', async () => {
    const html = await renderComponent(LocalTabs, {
      ...
      activeId: 'overview',
      items: [
        { id: 'overview', label: 'Overview', href: '/en/research?section=overview' },
        { id: 'methods', label: 'Methods', href: '/en/research?section=methods' },
      ],
    });
    ...
    expect(html).toMatch(/<a href="\/en\/research\?section=methods"[^>]*class="ui-local-tabs__tab"/);
    expect(html).not.toMatch(/<a href="\/en\/research\?section=methods"[^>]*aria-current=/);
  ...
  it('marks Button disabled and loading states without enabling interaction', async () => {
    ...
    expect(disabledHtml).toMatch(/<button[^>]*disabled[^>]*aria-disabled="true"/);
  ...
  it('associates Input label, hint, and error messaging for assistive tech', async () => {
    ...
    expect(html).toMatch(/<label[^>]*for="contact-email"[^>]*>Email address<\/label>/);
    expect(html).toMatch(/<input[^>]*id="contact-email"[^>]*aria-invalid="true"/);
    expect(html).toMatch(/aria-describedby="contact-email-hint contact-email-error"/);
    expect(html).toMatch(
  ...
  it('keeps focus-visible parity on Button using the shared focus token contract', () => {
    ...
    const foundation = readFileSync(path.join(repositoryRoot, 'src/styles/base.css'), 'utf8');

    expect(foundation).toMatch(/:where\(a, button, input, textarea, select, summary\):focus-visible/);
    expect(foundation).toMatch(/outline: 2px solid var\(--color-focus\)/);
    expect(source).toMatch(/:hover:not\(:disabled\):not\(\[aria-disabled='true'\]\)/);
