...
describe('PUBLIC-250 SEO gate', () => {
  it('declares sitemap integration, robots.txt, route registry, and validate:seo script', () => {
    ...
    expect(packageManifest.scripts['validate:seo']).toBe(
      'node scripts/validate-seo.mjs',
    )
    expect(
      readFileSync(path.join(repositoryRoot, 'public', 'robots.txt'), 'utf8'),
    ).toContain('Sitemap: /sitemap-index.xml')
    expect(
      existsSync(path.join(repositoryRoot, 'scripts', 'validate-seo.mjs')),
    ).toBe(true)
    expect(
      existsSync(path.join(repositoryRoot, 'scripts', 'seo-url.mjs')),
    ).toBe(true)
