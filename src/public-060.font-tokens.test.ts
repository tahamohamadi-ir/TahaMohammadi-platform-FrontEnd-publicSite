...
import { FONT_COMPUTED_PROBE_ROUTES, LOCALE_FONT_STACKS } from './test-harness/font-tokens';

const repositoryRoot = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');
const checklistPath = path.join(repositoryRoot, 'docs', 'quality', 'PUBLIC-060-FONT-COMPUTED-EVIDENCE.md');
const e2ePath = path.join(repositoryRoot, 'tests', 'e2e', 'public-060-font-computed.e2e.ts');
...
const harnessPath = path.join(repositoryRoot, 'src', 'test-harness', 'font-tokens.ts');
