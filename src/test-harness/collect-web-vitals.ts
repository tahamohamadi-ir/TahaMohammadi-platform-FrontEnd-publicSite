    ...
    try {
      new PerformanceObserver((list) => {
        for (const entry of list.getEntries()) {
          ...
          if (typeof event.interactionId === 'number' && event.interactionId > 0 && typeof event.duration === 'number') {
...
export async function readWebVitals(page: Page): Promise<WebVitalsSnapshot> {
  ...
  return page.evaluate(() => window.__tmVitals ?? { lcpMs: null, cls: 0, inpMs: null });
}
