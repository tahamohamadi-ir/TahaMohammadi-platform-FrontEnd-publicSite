...
export const CANONICAL_PUBLIC_OPENAPI_LF_SHA256 =
  '9f627d6e31c624cee7a7eaf123a33e3929bdc503b9619439e13f219afe149471'
...
export const PUBLIC_RESPONSE_FIXTURES: PublicResponseFixtureSpec[] = [
  ...
  {
    file: 'publication-detail.get.200.json',
    component: 'PublicationDetailOut',
  },
...
export const PUBLIC_ERROR_FIXTURES: PublicErrorFixtureSpec[] = [
  {
    file: 'profile-not-found.404.json',
    kind: 'json',
    matrix: 'profile-not-found',
  },
  {
    file: 'contact.post.error.json',
    kind: 'json',
    matrix: 'contact-json-failure',
  },
  {
...
export function readOpenApiArtifact(): OpenApiArtifact {
  return JSON.parse(
    readFileSync(PUBLIC_OPENAPI_PATH, 'utf8'),
  ) as OpenApiArtifact
}
...
export function hashOpenApiArtifactLf(
  filePath: string = PUBLIC_OPENAPI_PATH,
): string {
  return sha256Bytes(Buffer.from(canonicalLfBytes(filePath), 'utf8'))
}

export function hashOpenApiArtifactAccepted(
  filePath: string = PUBLIC_OPENAPI_PATH,
): string {
...
export function validateComponent(
  ...
  const unexpected = [...payloadKeys].filter(
    (key) => !component.properties.has(key),
  )
  ...
  if (options.itemComponent) {
    ...
    for (const entry of items) {
      ...
      const itemUnexpected = [...entryKeys].filter(
        (key) => !item.properties.has(key),
      )
      ...
      const itemMissing = [...item.required].filter(
...
export function assertProfileDetailShape(
  payload: Record<string, unknown>,
): void {
  const payloadKeys = new Set(Object.keys(payload))
  if (payloadKeys.size !== PROFILE_DETAIL_FIELDS.size) {
    const unexpected = [...payloadKeys].filter(
      (key) => !PROFILE_DETAIL_FIELDS.has(key),
    )
    const missing = [...PROFILE_DETAIL_FIELDS].filter(
  ...
  switch (spec.matrix) {
    case 'profile-not-found':
      if (typeof payload.detail !== 'string' || payload.detail.length === 0) {
        throw new Error(
          `${spec.file}: expected non-empty detail string for profile not found`,
        )
      }
      break
    case 'contact-json-failure':
      if (
        ...
        throw new Error(
          `${spec.file}: expected { ok: false, error: string } contact failure shape`,
        )
      }
      break
    default: {
