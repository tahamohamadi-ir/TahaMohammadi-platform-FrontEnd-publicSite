...
const checklistPath = path.join(repositoryRoot, 'docs', 'quality', 'PUBLIC-300-NO-JS-AUDIT.md');
const e2ePath = path.join(repositoryRoot, 'tests', 'e2e', 'public-300-nojs-crawl.e2e.ts');
const harnessPath = path.join(repositoryRoot, 'src', 'test-harness', 'no-js-audit.ts');
...
describe('PUBLIC-300 no-JS crawl audit scaffold', () => {
  it('covers all 23 static build routes (gateway, home, PF index, search)', () => {
    ...
    expect(NO_JS_AUDIT_ROUTES.map((route) => route.path)).toContain('/en/search/');
    expect(NO_JS_AUDIT_ROUTES.map((route) => route.path)).toContain('/fa/search/');
    expect(existsSync(harnessPath)).toBe(true);
  });

  it('includes PF-01 and PF-03..PF-08 index routes for both locales', () => {
    for (const capture of PAGE_FAMILY_INDEX_CAPTURES) {
      expect(NO_JS_AUDIT_ROUTES.some((route) => route.path === capture.path)).toBe(true);
    }

    for (const id of INDEX_ROUTE_IDS) {
      expect(NO_JS_AUDIT_ROUTES.some((route) => route.id.startsWith(id))).toBe(true);
    }
  });
