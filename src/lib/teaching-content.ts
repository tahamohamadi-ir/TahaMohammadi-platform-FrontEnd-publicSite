...
export type TeachingDetailModel =
  | { status: 'unavailable' }
  | { status: 'ready'; kind: TeachingDetailKind; record: CourseDetailOut | TalkDetailOut };

const teachingNavItem = primaryNav.find((item) => item.slug === 'teaching');

export function getTeachingRouteTitle(locale: Locale): string {
  return teachingNavItem?.label[locale] ?? (locale === 'en' ? 'Teaching' : 'تدریس');
}
...
export function getTeachingUnavailableCopy(locale: Locale): { title: string; message: string } {
  return locale === 'en'
    ? {
...
export function formatCourseCardMeta(course: CourseListOut): string {
  return [course.level, course.course_format, course.course_language, course.availability]
    .filter(Boolean)
    .join(' · ');
}
...
export async function fetchTeachingIndex(locale: Locale): Promise<TeachingIndexModel> {
  if (!canFetchPublicApi()) {
    return { status: 'unavailable' };
  }

  const [courses, talks] = await Promise.all([listCourses(locale), listTalks(locale)]);
...
export async function getTalkDetail(locale: Locale, slug: string): Promise<TalkDetailOut | null> {
  if (!canFetchPublicApi()) return null;
  try {
...
export async function listTeachingDetailSlugs(locale: Locale): Promise<string[]> {
  const [courses, talks] = await Promise.all([listCourses(locale), listTalks(locale)]);
...
export async function resolveTeachingAlternateAvailability(locale: Locale): Promise<boolean> {
