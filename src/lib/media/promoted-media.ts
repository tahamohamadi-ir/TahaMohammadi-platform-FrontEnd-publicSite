...
import type { Locale, MediaSlot, PromotedAssetRecord, Theme } from './promoted-media-registry';
import {
  altForLocale,
...
export { altForLocale, getPromotedAssetRecord, PROMOTED_ASSET_REGISTRY } from './promoted-media-registry';
export {
...
function assertSourceHash(record: PromotedAssetRecord, source: ImageMetadata) {
  ...
  if (digest !== record.sourceSha256) {
    throw new Error(`Source hash drift for ${record.id}: expected ${record.sourceSha256}, got ${digest}`);
  }
  if (source.width !== record.intrinsic.width || source.height !== record.intrinsic.height) {
    throw new Error(
...
for (const record of Object.values(PROMOTED_ASSET_REGISTRY)) {
  ...
  if (!source) {
    throw new Error(`Missing Astro source import for promoted asset ${record.id}`);
  }
...
export function getPromotedMediaAlt(id: string, expectedSlot: MediaSlot, locale: Locale): string {
