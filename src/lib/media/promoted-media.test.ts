...
import {
  DERIVATIVE_MANIFEST,
  DEFERRED_DERIVATIVE_ENTRIES,
} from './derivative-manifest'
import {
...
function sha256File(relativePath: string) {
  return crypto
    .createHash('sha256')
    .update(readFileSync(path.join(mediaRoot, relativePath)))
    .digest('hex')
}

describe('WP-30 promoted media registry', () => {
  it('registers every approved runtime asset id with exact authority checksums', () => {
    expect(Object.keys(PROMOTED_ASSET_REGISTRY).sort()).toEqual(
      [...RUNTIME_ASSET_IDS].sort(),
    )
  ...
  it('enforces slot placement when resolving records', () => {
    expect(() =>
      getPromotedAssetRecord('portal-orbit-light', 'gateway.atmosphere'),
    ).toThrow(/registered for home\.hero\.atmosphere/)
    expect(
      getPromotedAssetRecord('portal-centered-light', 'gateway.atmosphere').id,
    ).toBe('portal-centered-light')
  })
  ...
  it('uses decorative alt="" for atmosphere, rails, graph backplates, and brand favicon', () => {
    for (const id of [
      ...
      expect(PROMOTED_ASSET_REGISTRY[id].semantics).toEqual({
        kind: 'decorative',
        alt: '',
      })
    }
  ...
  it('requires consumer-supplied localized alt for project previews', () => {
    ...
    expect(() => altForLocale(project, 'en')).toThrow(
      /consumer-supplied localized alt/,
    )
    expect(() =>
  ...
  it('pins correct intrinsic dimensions for atmosphere and preview groups', () => {
    ...
    expect(PROMOTED_ASSET_REGISTRY['brand-primary'].intrinsic).toEqual({
      width: 256,
      height: 233,
    })
    expect(PROMOTED_ASSET_REGISTRY['brand-favicon'].intrinsic).toEqual({
      width: 64,
      height: 64,
    })
    expect(
      PROMOTED_ASSET_REGISTRY['home-graph-backplate-light'].intrinsic,
    ).toEqual({
      width: 1254,
      height: 1254,
    })
    expect(
      PROMOTED_ASSET_REGISTRY['home-graph-backplate-dark'].intrinsic,
    ).toEqual({
      width: 1254,
      height: 1254,
    })
  ...
  it('assigns atmosphere, preview, and graph backplate transform width sets', () => {
    expect(
      PROMOTED_ASSET_REGISTRY['portal-centered-dark'].transform.widths,
    ).toEqual(ATMOSPHERE_WIDTHS)
    expect(
      PROMOTED_ASSET_REGISTRY['project-data-architecture'].transform.widths,
    ).toEqual(PREVIEW_WIDTHS)
    expect(
      PROMOTED_ASSET_REGISTRY['home-graph-backplate-light'].transform.widths,
    ).toEqual(GRAPH_BACKPLATE_WIDTHS)
    expect(
      PROMOTED_ASSET_REGISTRY['blog-coral-stairs'].transform.formats,
    ).toEqual(PROMOTED_FORMATS)
    expect(
      PROMOTED_ASSET_REGISTRY['home-graph-backplate-dark'].transform.widths,
    ).not.toContain(1254)
  })

  it('maps projects and rails to definitive runtime asset ids', () => {
    expect(HOME_PROJECT_ASSET_BY_SLUG['pars-sql-vtd-edge']).toBe(
      'project-data-architecture',
    )
    expect(
      HOME_PROJECT_ASSET_BY_SLUG['organizational-dashboard-research'],
    ).toBe('project-dashboard-systems')
  ...
  it('documents active derivative manifest entries for all runtime assets', () => {
    ...
    expect(
      DERIVATIVE_MANIFEST.every((entry) => entry.status === 'active'),
    ).toBe(true)
    expect(DEFERRED_DERIVATIVE_ENTRIES).toHaveLength(0)

    const graphLight = DERIVATIVE_MANIFEST.find(
      (entry) => entry.assetId === 'home-graph-backplate-light',
    )
  ...
  it('documents runtime loader rejection for deferred ids', () => {
    const source = readFileSync(
      path.join(repositoryRoot, 'src/lib/media/promoted-media.ts'),
      'utf8',
    )
  ...
  it('resolves mediaRoot by preferring project-root src/assets/media over import.meta fallback', () => {
    ...
    const resolvedPrerenderRoot = resolveMediaRoot(
      repositoryRoot,
      prerenderChunkUrl,
    )
    expect(resolvedPrerenderRoot).toBe(mediaRoot)

    const nonExistentCwd = path.join(
      repositoryRoot,
      'non-existent-prerender-cwd',
    )
    const sourceMetaUrl = pathToFileURL(
      path.join(repositoryRoot, 'src/lib/media/promoted-media.ts'),
    ).href
