...
describe('PUBLIC-230 site settings helpers', () => {
  it('detects published contact details and form availability', () => {
    ...
    expect(isContactFormAvailable({ ...contact, formEnabled: false })).toBe(
      false,
    )
  })
...
describe('PUBLIC-230 contact content helpers', () => {
  it('returns localized route titles and unavailable copy', () => {
    ...
    expect(getContactUnavailableCopy('en').message).toContain(
      'not available yet',
    )
  })
  ...
  it('parses contact form return query state', () => {
    ...
    const failed = parseContactFormState(
      new URLSearchParams(
        'sent=0&name=Ada&email=ada@example.com&message=Hello',
      ),
    )
...
describe('PUBLIC-230 contact form adapter', () => {
  ...
  it('maps JSON success and failure responses', () => {
    expect(
      mapContactSubmitResponse(200, 'application/json', '{"ok":true}', fields),
    ).toEqual({
      kind: 'success',
    })
    expect(
      mapContactSubmitResponse(
        400,
        'application/json',
        '{"ok":false,"error":"bad"}',
        fields,
