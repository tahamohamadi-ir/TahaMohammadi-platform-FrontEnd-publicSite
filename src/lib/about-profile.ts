...
function resolveApiBaseUrl(): string {
  return String(import.meta.env.PUBLIC_API_BASE_URL ?? '').trim().replace(/\/+$/, '');
}
...
export function getAboutUnavailableCopy(locale: Locale): { title: string; message: string } {
  return locale === 'en'
    ? {
...
export async function fetchAboutProfile(locale: Locale): Promise<AboutPageModel> {
  ...
  try {
    ...
    if (error instanceof PublicApiError && (error.kind === 'network' || error.kind === 'validation')) {
      return { status: 'unavailable' };
    }
    return { status: 'unavailable' };
  }
