...
export type ResearchTopicDetailOut = components['schemas']['ResearchTopicDetailOut'];
...
export type ResearchDetailModel =
  | { status: 'unavailable' }
  | { status: 'ready'; kind: ResearchDetailKind; record: ResearchTopicDetailOut | ResearchStatementOut };

const researchNavItem = primaryNav.find((item) => item.slug === 'research');

export function getResearchRouteTitle(locale: Locale): string {
  return researchNavItem?.label[locale] ?? (locale === 'en' ? 'Research' : 'پژوهش');
}

export function getResearchUnavailableCopy(locale: Locale): { title: string; message: string } {
  return locale === 'en'
    ? {
...
export function getResearchEmptyTopicsCopy(locale: Locale): { title: string; message: string } {
  return locale === 'en'
    ? {
        title: 'No research topics yet',
        message: 'Published research topics will appear here when they are available.',
      }
    : {
        title: 'هنوز موضوع پژوهشی منتشر نشده است',
        message: 'موضوعات پژوهشی منتشرشده پس از آماده‌شدن در اینجا نمایش داده می‌شوند.',
      };
}
...
async function fetchAllPagedItems<T extends { published_at?: string | null }>(
  ...
  while (true) {
    ...
    const payload = await fetchJson<{ count: number; items: T[] }>(`${pathPrefix}${query}`);
...
export async function listResearchTopics(locale: Locale): Promise<ResearchTopicListOut[]> {
...
export async function listResearchProjects(locale: Locale): Promise<ProjectListOut[]> {
  if (!canFetchPublicApi()) return [];
  try {
    return await fetchAllPagedItems<ProjectListOut>(`/api/research/projects/${locale}`);
  } catch {
    return [];
  }
}

export async function listResearchStatements(locale: Locale): Promise<ResearchStatementOut[]> {
  if (!canFetchPublicApi()) return [];
  try {
...
export async function fetchResearchIndex(locale: Locale): Promise<ResearchIndexModel> {
...
export async function listResearchDetailSlugs(locale: Locale): Promise<string[]> {
...
export async function resolveResearchAlternateAvailability(locale: Locale): Promise<boolean> {
