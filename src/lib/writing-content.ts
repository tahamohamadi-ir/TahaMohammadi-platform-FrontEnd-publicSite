...
export function getWritingUnavailableCopy(locale: Locale): { title: string; message: string } {
  return locale === 'en'
    ? {
...
export function formatBookCardMeta(book: BookListOut): string {
  return [book.authors, book.publisher, book.publication_date, book.access_state]
    .filter(Boolean)
    .join(' · ');
}
...
export async function fetchWritingIndex(locale: Locale): Promise<WritingIndexModel> {
  if (!canFetchPublicApi()) {
    return { status: 'unavailable' };
  }

  const [articles, books] = await Promise.all([listArticles(locale), listBooks(locale)]);
...
export async function resolveWritingAlternateAvailability(locale: Locale): Promise<boolean> {
