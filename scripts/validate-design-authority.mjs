...
if (fs.existsSync(centralRoot)) {
  ...
  console.log(
    `PASS: snapshot validated locally and against central authority (${componentNames.length} components, ${templateIds.length} templates).`,
  )
} else {
  console.log(
    `PASS: standalone snapshot validated (${componentNames.length} components, ${templateIds.length} templates); central authority unavailable.`,
  )
}
