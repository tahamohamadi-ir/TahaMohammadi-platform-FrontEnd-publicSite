...
const siteOrigin = (process.env.PUBLIC_SITE_URL ?? 'http://127.0.0.1:4321').replace(/\/+$/, '');
...
function collectSitemapUrls() {
  ...
  const childPaths = [...indexXml.matchAll(/<loc>([^<]+)<\/loc>/g)].map(([, loc]) => loc);
...
function validateLocalePage(relativePath, html) {
  ...
  if (canonical !== expectedCanonical) {
    fail(`${relativePath} canonical ${canonical} does not match ${expectedCanonical}`);
  }
  ...
  const alternateAvailable = alternates.some((link) => link.hreflang === alternateLocale);
  ...
  for (const expected of expectedAlternates) {
    const actual = alternates.find((link) => link.hreflang === expected.hreflang);
    if (!actual) {
  ...
  const unexpected = alternates.filter(
    (link) => !expectedAlternates.some((expected) => expected.hreflang === link.hreflang),
  );
  if (unexpected.length) {
    fail(`${relativePath} has unexpected hreflang links: ${unexpected.map((link) => link.hreflang).join(', ')}`);
  }
}
