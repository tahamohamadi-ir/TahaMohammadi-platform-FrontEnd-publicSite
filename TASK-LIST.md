...
- [~] **PUBLIC-190** Home `/{locale}/` — **structure complete; visual acceptance open**. PF-02 honest empty-shell detail @ `798e8b2`; Phase 8 Home split hero + Gateway 200% @ `84e83e9`; compare **43/48** ready confirmed by owner `review:visual` run @ `675a08f` (39 index + 4 PF-02 empty-shell; 5 capture-only remain); About/Contact/Research leftover chrome (PF-05/07/08) @ `c7581b7` (390 overflow fix @ `05c87a0`). Coordination remediation pack: `Docs/10-tracking/PUBLIC-190-asset-prompts/` (owner regenerate decorative masters), `PUBLIC-190-VISUAL-REMEDIATION-PLAN.md`, `PUBLIC-190-IMPLEMENTATION-REQUIREMENTS.md`. **Open:** owner asset handback, CMS Category A, Category D nav labels, manual compare. Independent QA and owner approval remain required — **do not mark PASS**.
