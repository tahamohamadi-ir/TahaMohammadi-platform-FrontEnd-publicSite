...
- [~] **PUBLIC-190** Home `/{locale}/` — **structure complete; visual acceptance open**. Phase 8 Home split hero + Gateway 200% affordance (tip pending commit). Compare **39/48** @ `c14508a`. Coordination remediation pack: `Docs/10-tracking/PUBLIC-190-asset-prompts/` (owner regenerate decorative masters), `PUBLIC-190-VISUAL-REMEDIATION-PLAN.md`, `PUBLIC-190-IMPLEMENTATION-REQUIREMENTS.md`. **Open:** owner asset handback, CMS Category A, PF-02 route, manual compare. Independent QA and owner approval remain required — **do not mark PASS**.
