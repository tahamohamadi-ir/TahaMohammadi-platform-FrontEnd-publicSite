...
const SHELL_MUTED_LABEL_SELECTORS = '.site-header__brand-role, .site-footer__nav-title';

test.describe('PUBLIC-150 shell muted label contrast', () => {
  for (const theme of ['light', 'dark'] as const) {
    test(`passes color contrast for shell muted labels in ${theme} theme @a11y`, async ({ page }) => {
