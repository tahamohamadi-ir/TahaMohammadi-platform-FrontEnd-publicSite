...
const tokens = JSON.parse(
  readFileSync(
    path.join(repositoryRoot, 'contracts', 'design-authority', 'tokens.json'),
    'utf8',
  ),
) as {
...
test.describe('WP-10 foundation acceptance', () => {
  const matrix = [
    {
      ...
      name: 'Gateway desktop light',
      path: '/',
      theme: 'light',
      width: 1440,
    },
    {
      id: 'gateway-1440-dark',
      name: 'Gateway desktop dark',
      path: '/',
      theme: 'dark',
      width: 1440,
    },
    {
      id: 'gateway-390-light',
      name: 'Gateway mobile light',
      path: '/',
      theme: 'light',
      width: 390,
    },
    {
      id: 'gateway-390-dark',
      name: 'Gateway mobile dark',
      path: '/',
      theme: 'dark',
      width: 390,
    },
    {
      id: 'home-en-1440-light',
    ...
    {
      id: 'home-en-1440-dark',
      name: 'Home EN desktop dark',
      path: '/en/',
      locale: 'en',
      dir: 'ltr',
      theme: 'dark',
    ...
    {
      id: 'home-fa-390-light',
      name: 'Home FA mobile RTL light',
      path: '/fa/',
      locale: 'fa',
      dir: 'rtl',
      theme: 'light',
    ...
    {
      id: 'home-en-768-light',
      name: 'Home EN tablet light',
      path: '/en/',
      locale: 'en',
      dir: 'ltr',
      theme: 'light',
    ...
    {
      id: 'home-fa-768-light',
      name: 'Home FA tablet RTL light',
      path: '/fa/',
      locale: 'fa',
      dir: 'rtl',
      theme: 'light',
  ...
  for (const target of matrix) {
    test(`WP-40 screenshot discovery: ${target.name} has the required locale, theme, H1, overflow, and capture evidence @later-packet-acceptance`, async ({
      page,
    }) => {
      ...
      await page.screenshot({
        path: `test-results/visual/wp40-${target.id}.png`,
        fullPage: true,
      })
      await expect
        .poll(() =>
  ...
  test('derives every computed semantic role from the pinned light and dark token snapshot @visual', async ({
    page,
  }) => {
    await page.goto('/en/')
    ...
    for (const [theme, semanticRoles] of [
      ...
      await page.evaluate(
        (requestedTheme) => window.__tmApplyTheme(requestedTheme),
        theme,
      )
  ...
  test('keeps multiple theme controls independent and cycles system light dark @foundation', async ({
    page,
  }) => {
    ...
    await page.addInitScript(() => {
      ...
      window.addEventListener('tm-themechange', (event) =>
        window.__tmThemeEvents.push(event.detail),
      )
    })
    await page.goto('/en/')
    ...
    await expect
      .poll(() => page.evaluate(() => window.__tmThemeEvents))
      .toEqual([{ requested: 'system', resolved: 'dark' }])
    await page.evaluate(() => {
    ...
    await expect
      .poll(() => page.evaluate(() => localStorage.getItem('tm-theme')))
      .toBe('light')
    ...
    await expect(controls.first()).toHaveAttribute('aria-pressed', 'mixed')
    await expect
      .poll(() => page.evaluate(() => window.__tmThemeEvents.at(-1)))
      .toEqual({ requested: 'system', resolved: 'light' })
    await page.reload()
    await expect(page.locator('html')).toHaveAttribute(
      'data-theme-requested',
      'system',
  ...
  test('persists an explicit requested mode and emits one resolution event after reload @foundation', async ({
    page,
  }) => {
    await page.addInitScript(() => {
      window.__tmThemeEvents = []
    ...
    await expect
      .poll(() => page.evaluate(() => window.__tmThemeEvents))
      .toEqual([{ requested: 'light', resolved: 'light' }])
  })
})
