...
import {
  ...
  LOCALE_FONT_STACKS,
} from '../../src/test-harness/font-tokens'

test.describe('PUBLIC-060 locale font computed styles', () => {
  for (const route of FONT_COMPUTED_PROBE_ROUTES) {
    test(`PUBLIC-060 ${route.label} body and display fonts match locale tokens @foundation`, async ({
      page,
    }) => {
      ...
      await expect(page.locator('body')).toHaveCSS(
        'font-family',
        stack.body.computedPattern,
      )
      await expect(page.getByRole('heading', { level: 1 })).toHaveCSS(
        'font-family',
        stack.display.computedPattern,
      )
    })
  }
  ...
  test('PUBLIC-060 retains locale font stacks at 200% root font size @foundation', async ({
    page,
  }) => {
    await page.goto('/fa/')
    await page.evaluate(() => {
