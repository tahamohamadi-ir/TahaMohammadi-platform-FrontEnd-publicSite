...
async function assertLocaleShell(page: Page, route: NoJsAuditRoute) {
  ...
  if (aboutLabel) {
    await expect(
      desktopNav.getByRole('link', { name: aboutLabel }),
    ).toBeVisible()
  }
...
test.describe('PUBLIC-300 no-JS crawl audit', () => {
  for (const route of NO_JS_AUDIT_ROUTES) {
    test(`PUBLIC-300 ${route.id} readable without JavaScript @nojs`, async ({
      browser,
    }) => {
