...
import {
  ...
  readWebVitals,
  waitForInteractionTiming,
} from '../../src/test-harness/collect-web-vitals'
import {
...
const evidenceOutputDir = path.resolve(
...
test.describe('PUBLIC-290 performance budget', () => {
  for (const route of PERFORMANCE_PROBE_ROUTES) {
    test(`PUBLIC-290 ${route.label} LCP/CLS within local budget @performance`, async ({
      page,
    }) => {
      ...
      expect(vitals.lcpMs!, `${route.id} LCP`).toBeLessThanOrEqual(
        PERFORMANCE_BUDGET.lcpMs,
      )
      expect(vitals.cls, `${route.id} CLS`).toBeLessThanOrEqual(
  ...
  test('PUBLIC-290 locale font preloads match BaseLayout body/display faces @performance', async ({
    page,
  }) => {
    for (const locale of ['en', 'fa'] as const) {
      ...
      expect(hrefs, `${locale} preload count`).toHaveLength(
        LOCALE_FONT_PRELOADS[locale].length,
      )
      ...
      for (const href of hrefs) {
        ...
        expect(response.headers()['content-type']).toMatch(
          /font|woff2|octet-stream/i,
        )
      }
    }
  })

  test('PUBLIC-290 bundled CSS retains font-display swap @performance', async ({
    page,
  }) => {
    await page.goto('/en/')
...
test('PUBLIC-290 home EN theme toggle INP within local budget @performance', async ({
  page,
}) => {
  test.setTimeout(120_000)
  await installWebVitalsCollector(page)
  ...
  expect(vitals.inpMs!, 'home-en theme toggle INP').toBeLessThanOrEqual(
    PERFORMANCE_BUDGET.inpMs,
  )
