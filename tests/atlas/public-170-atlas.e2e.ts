...
test.describe('PUBLIC-170 visual atlas', () => {
  ...
  test.beforeAll(async () => {
    const started = await startDistStaticServer(
  ...
  test('serves the atlas route when DESIGN_ATLAS=1 @atlas', async ({
    page,
  }) => {
    await page.goto(`${atlasBaseUrl}/_design`)
    await expect(page.locator('[data-visual-id="VisualAtlas"]')).toBeVisible()
    await expect(
      page.getByRole('heading', { level: 1, name: 'Visual Atlas' }),
    ).toBeVisible()
  })

  test('resolves distinct light and dark token values on the live theme probe @atlas', async ({
    page,
  }) => {
    await page.goto(`${atlasBaseUrl}/_design`)
  ...
  test('applies the reduced-motion contract to the motion probe @atlas', async ({
    page,
  }) => {
  ...
  test('renders stable theme and motion specimen visual ids @atlas', async ({
    page,
  }) => {
    await page.goto(`${atlasBaseUrl}/_design`)

    await expect(
      page.locator('[data-visual-id="AtlasThemeSpecimen-light"]'),
    ).toBeVisible()
    await expect(
      page.locator('[data-visual-id="AtlasThemeSpecimen-dark"]'),
    ).toBeVisible()
    await expect(
      page.locator('[data-visual-id="AtlasThemeSpecimen-live"]'),
    ).toBeVisible()
    await expect(
      page.locator('[data-visual-id="AtlasReducedMotionSpecimen"]'),
    ).toBeVisible()
    await expect(
      page.locator(
        '[data-visual-id="AtlasSpecimen-IconButton"] svg.lucide-menu',
      ),
    ).toHaveCount(2)
  })
})
